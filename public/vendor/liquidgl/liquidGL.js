...
(() => {
  ...
  function applyLeCuvierCloneFixes(clonedDoc) {
    ...
    const normalizeColorFunction = (value) => {
      ...
      const parseChannel = (channel, max = 255) => {
        if (typeof channel !== "string") return 0;

        const trimmed = channel.trim();
        const numeric = parseFloat(trimmed);

        if (!Number.isFinite(numeric)) return 0;
        if (trimmed.endsWith("%")) return Math.max(0, Math.min(max, (numeric / 100) * max));

        return Math.max(0, Math.min(max, numeric * max));
      };

      const parseAlpha = (alpha) => {
        if (typeof alpha !== "string") return 1;

        const trimmed = alpha.trim();
        const numeric = parseFloat(trimmed);

        if (!Number.isFinite(numeric)) return 1;
        if (trimmed.endsWith("%")) return Math.max(0, Math.min(1, numeric / 100));
      ...
      return value.replace(
        ...
        (_match, red, green, blue, alpha) => {
          const r = Math.round(parseChannel(red));
          const g = Math.round(parseChannel(green));
          const b = Math.round(parseChannel(blue));
    ...
    style.textContent = `
      ...
      .club-hero__svg,
      .club-hero__svg *,
      .club-hero__society-text {
        color: rgba(161, 152, 131, 0.12) !important;
        fill: rgba(161, 152, 131, 0.12) !important;
        stroke: rgba(161, 152, 131, 0.12) !important;
        outline-color: rgba(161, 152, 131, 0.12) !important;
        text-decoration-color: rgba(161, 152, 131, 0.12) !important;
        caret-color: rgba(161, 152, 131, 0.12) !important;
        column-rule-color: rgba(161, 152, 131, 0.12) !important;
      }
  ...
  class liquidGLRenderer {
    constructor(snapshotSelector, snapshotResolution = 1.0, options = {}) {
      ...
      // Le Cuvier nav: initialize before the first capture to avoid a NaN scale.
      ...
      const attemptCapture = async (
        ...
        try {
          ...
          const ignoreElementsFunc = (element) => {
            ...
            if (
              element.matches &&
              element.matches("select, option, optgroup, input, textarea, button")
            ) {
              return true;
            }
          ...
          const proxy = this.options.proxy || null;
          const useProxy = typeof proxy === "string" && proxy.length > 0;
          ...
          const snapCanvas = await html2canvas(this.snapshotTarget, {
            ...
            onclone: applyLeCuvierCloneFixes,
          });
